...
- **가장자리 드롭·셸프 패널** — 화면 오른쪽 끝 탭에 끌어다 놓으면 변환(위) 또는 보관(아래)
...
| 드롭·셸프 패널 | ✅ | ✅ 같은 크기·동작(X11 · Wayland 는 XWayland — Wayland 는 아직 검증 전) |
...
macOS 의 통합 드롭·셸프 패널과 같습니다. 화면 **오른쪽 가장자리 가운데에 얇은 탭**으로 접혀 있다가(설정에서
왼쪽으로 옮길 수 있음 — 기본 Dock 이 왼쪽이라 오른쪽이 기본), 파일을 끌어오거나 마우스를 잠시 대면 펼쳐집니다.

- **위 · 변환 드롭존** — 영상·이미지를 놓으면 곧바로 변환합니다(우클릭 "Sizer로 변환"과 같음). 변환할 수 없는
  형식이면 주황색으로 거부합니다.
- **아래 · 보관 트레이** — Files 에서 끌어와 **임시로 담아 두는** 곳입니다. 원본을 참조하므로 복사본이 생기지 않고,
  원하는 곳으로 **끌어내면** Files 가 이동/복사합니다(이동으로 꺼낸 항목은 목록에서 사라집니다).
  변환이 끝난 결과도 맨 앞에 **NEW** 로 얹힙니다.

| 조작 | 동작 |
|---|---|
| 클릭 · `Ctrl`/`Shift`+클릭 | 선택 · 여러 개 선택 |
| `스페이스` | **훑어보기**(GNOME Sushi) |
| 더블클릭 · `Enter` | 기본 앱으로 열기 |
| 우클릭 | 열기 · 훑어보기 · 폴더에서 보기 · 정보 보기 · 경로 이름 복사 · 목록에서 제거 |
| `Delete` · `Ctrl+C` | 목록에서 제거 · 경로 이름 복사 |
| 마우스를 잠시 올려 두기 | 경로·크기·수정일 |

- 트레이 메뉴 **드롭·셸프 패널** 또는 설정 창 **일반 → 드롭 & 셸프** 에서 켜고 끄고 위치를 바꿉니다.
- **단축키로 열기**: 설정 → 키보드 → 키보드 단축키 보기 → 사용자 지정 단축키에서 명령
  `sizer-tray --toggle-panel` 을 원하는 키에 지정하세요. 누르면 펼쳐지고, 한 번 더 누르거나 `Esc` 로 접힙니다.
- 마우스가 있는 모니터의 가장자리를 따라다니므로 다중 모니터에서도 닿습니다.
- 보관 목록은 트레이가 떠 있는 동안만 유지됩니다(macOS 와 같음).
...
- **메뉴**: 상태(감시 중 / 변환 중: 파일명 · 대기 N / 일시정지), 최근 변환 8개(누르면 결과 열기, 실패는 로그),
  드롭·출력 폴더 열기, 지금 다시 스캔, 감시 일시정지/재개, 드롭·셸프 패널, 모니터 꺼짐 방지, 설정…, 로그 열기, Sizer 종료
...
| `sizer-tray` | 상단 바 아이콘 + 드롭·셸프 패널(로그인 시 자동 시작 — 직접 쓸 일은 거의 없음) |
| `sizer-tray --toggle-panel` | 드롭·셸프 패널 펼치기/접기(사용자 지정 단축키용) |
...
| `panel.enabled` · `panel.side` · `panel.addResults` | `true` · `"right"` · `true` | 드롭·셸프 패널 표시 · 위치(`left`/`right`) · 변환 결과를 보관 트레이에 추가 |
...
- **드롭·셸프 패널이 없음**: 트레이 메뉴의 **드롭·셸프 패널** 이 켜져 있는지 보세요. 트레이를 터미널에서
  `sizer-tray` 로 실행하면 패널 오류가 출력됩니다.
