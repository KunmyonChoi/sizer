...
메뉴바 → **파일 셸프 표시** 를 켜면 임시 파일 보관대가 나타납니다. 평상시에는 화면 모서리에
**정사각형 아이콘으로 최소화**되어 있다가(드래그로 옮기면 화면 모서리에 **자석처럼 스냅**), 마우스를
올리거나 파일을 드래그해 오면 어느 모서리에 있든 **안쪽으로 펼쳐집니다**(화면 점유 최소화).
Finder에서 파일들을 셸프에 드롭해 썸네일로 모아 두고, 원하는 위치로 이동한 뒤 셸프의 카드를 그 위치로
**드래그해 꺼내면** 파일이 복사/이동됩니다(⌥로 복사 강제 등 macOS 표준). 원본을 **참조**하므로 복사본이
쌓이지 않고, 원본이 옮겨진 항목은 자동으로 정리됩니다. 흩어진 파일을 한 번에 모아 옮길 때 편리합니다.
