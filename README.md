...
- 창이 걸쳐 있는 **그 화면** 기준으로 배치합니다. **멀티 모니터에서 같은 방향키를 반복하면 인접 모니터로 넘어갑니다**(Magnet식):
  좌측 반 → (이미 좌측 반이면) 왼쪽 모니터의 우측 반 → 그 화면 좌측 반 → … (끝 모니터에서 멈춤). 우측도 대칭.
  크기가 다르거나 어긋나게 배치된 모니터에서도 목적지 화면 크기에 맞춰 정확히 리사이즈됩니다.
