...
<p align="center">
  ...
  전역 단축키 <b>창 스냅</b>으로 현재 창을 화면 좌/우 절반·최대화로 정렬하는 기능도 있습니다.
</p>
...
- **일반**: 드롭/출력/완료/실패 폴더 변경, 로그인 시 자동 시작, 알림 on/off, **드롭 타겟 변환 후 출력 폴더 열기(기본 켬)**, **오래된 원본 자동 삭제(processed, 기본 켬·30일)**, **모니터 꺼짐 방지**(전환 단축키)
...
- **창 스냅**: 창 스냅 사용(기본 켬), **좌측 반/우측 반/최대화 단축키**(기본 `⌃⌥←` / `⌃⌥→` / `⌃⌥↩`), 손쉬운 사용 권한 상태·바로가기
...
- 창이 걸쳐 있는 **그 화면** 기준으로 배치하므로 다중 모니터에서도 자연스럽습니다.
- 설정 → **창 스냅**에서 사용 여부, 단축키 재지정, 권한 상태를 관리합니다.
