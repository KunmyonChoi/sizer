...
화면 가장자리 **드롭·셸프 패널**, 상단 바 아이콘(상태·최근 변환·일시정지·모니터 꺼짐 방지), 설정 창,
Files 우클릭 **"Sizer로 변환"**, `열기`·`폴더에서 보기` 버튼이 달린 알림, `sizer` 터미널 명령. 창 스냅은 macOS 전용입니다.
...
모두 **전역 단축키**입니다 — 어떤 앱을 쓰고 있든 동작합니다. 설정에서 자유롭게 재지정하거나 지울 수 있습니다.

| 기본 단축키 | 동작 | 재지정 |
|---|---|---|
| `⌃⌥←` `⌃⌥→` | 창을 좌/우 **절반**으로. 이미 그 절반이면 한 번 더 눌러 **그 방향 화면으로** | 설정 → 창 스냅 |
| `⌃⌥⇧←` `⌃⌥⇧→` | 창을 좌/우 **1/3 ↔ 2/3** 토글 (와이드 화면 3분할) | 설정 → 창 스냅 |
| `⌃⌥↩` | 창을 **최대화** (메뉴바·Dock 제외) | 설정 → 창 스냅 |
| `⌃⌥⌘←` `⌃⌥⌘→` | 창을 비율 유지한 채 **이전/다음 화면으로** | 설정 → 창 스냅 |
| *(지정 안 됨)* | **드롭·셸프 패널** 열기/닫기 | 설정 → 드롭 & 셸프 |
| *(지정 안 됨)* | **모니터 꺼짐 방지** 켜기/끄기 | 설정 → 꺼짐 방지 |

- **창 스냅 네 줄은 손쉬운 사용(Accessibility) 권한이 필요합니다** — 다른 앱의 창을 옮기는 macOS 공통 제약입니다.
  자세한 동작과 멀티 모니터 규칙은 [창 스냅](#창-스냅-윈도우-타일링)을 보세요.
- `⌃←` / `⌃→` 는 macOS 스페이스 전환과 충돌하므로 여기에 `⌥` 를 더해 피했습니다(Rectangle과 같은 조합).
- 다른 타일링 앱(Magnet · Rectangle 등)이 같은 조합을 먼저 잡고 있으면 Sizer 쪽이 등록되지 않습니다.
  먼저 등록한 앱이 이기므로, 안 먹으면 설정에서 다른 키로 바꾸세요.
...
파일이 보관 트레이 맨 앞에 **NEW** 배지와 함께 얹혀, 바로 원하는 곳으로 끌어낼 수 있습니다. 패널을 거치지 않고
**Finder에서 드롭 폴더에 직접 넣은 파일**도 변환 결과가 똑같이 트레이에 얹힙니다(출력 폴더 자동 열기는 패널로
넣은 경우만).

보관 트레이의 파일은 Finder처럼 다룰 수 있습니다.

| 조작 | 동작 |
|---|---|
| 선택 후 `스페이스` | **훑어보기**(Quick Look) — 이미지·영상 미리보기. 다시 누르거나 `Esc` 로 닫고, `←` `→` 로 다른 항목 |
| 더블클릭 | 기본 앱으로 **열기**(여러 개 선택 중이면 모두) |
| 마우스를 잠시 올려 두기 | 파일명·**절대경로**·크기·생성일·수정일 표시 |
| 우클릭 | **열기 · 훑어보기 · Finder에서 보기 · 정보 보기 · 경로 이름 복사**(파일명을 포함한 절대경로, 여러 개면 줄마다 하나) |

- 정보 보기로 연 창에서는 경로를 드래그해 일부만 선택·복사할 수도 있습니다.
- 메뉴·훑어보기·정보 창이 떠 있는 동안에는 패널이 접히지 않습니다.
...
키 목록은 [단축키](#단축키)에 모아 두었습니다. 여기서는 동작을 설명합니다.
